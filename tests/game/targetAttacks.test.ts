...
import { createDefaultProfile, loadProfile, CURRENT_PROFILE_VERSION } from '../../src/profiles';
import {
...
const fight = (profile: ReturnType<typeof geared>, attacks: boolean, seed = 3) =>
  runProfileBatch({
    ...profile,
...
describe('a target that stands still', () => {
  ...
  it('leaves the player untouched', () => {
    ...
    expect(log.some((line) => /Training Dummy Melee/.test(line))).toBe(false);
  });
});
...
describe('a target that swings back', () => {
  it('is armed with the placeholder boss melee', () => {
    ...
    expect(dummy.weapons.mainHand?.baseDamage).toBe(PLACEHOLDER_BOSS_SWING_DAMAGE);
    expect(dummy.weapons.mainHand?.swingTimerMs).toBe(PLACEHOLDER_BOSS_SWING_SECONDS * 1000);
  ...
  it('actually hits the player', () => {
    ...
    expect(log.some((line) => /Training Dummy Melee/.test(line))).toBe(true);
  });
  ...
  it('resolves against the attacks-received table, not the melee one', () => {
    ...
    const incoming = log.filter((line) => /Training Dummy Melee/.test(line));
  ...
  it('reports no overkill on a character who survived', () => {
    const log = fight(geared('dual_wield'), true).representative.combatLog;
    const incoming = log.filter((line) => /Training Dummy Melee hits/.test(line));
...
describe('what being attacked brings to life', () => {
  it('lets a shield warrior BLOCK, and says so in the log', () => {
    ...
    expect(log.some((line) => /Training Dummy Melee hits .* \(blocked\)/.test(line))).toBe(true);
  });
  ...
  it('never casts Revenge when nothing is attacking', () => {
    ...
    // needing to know why.
    const log = fight(geared('one_hand_shield', { shield_slam: 1 }), false).representative.combatLog;
  ...
  it('puts Revenge in the list for every melee style', () => {
    for (const style of ['dual_wield', 'two_hander', 'one_hand_shield'] as const) {
      ...
      expect(rotation.entries.map((entry) => entry.abilityId)).toContain('revenge');
    }
  });
...
describe('profile version 6', () => {
  it('migrates a version 5 profile to a target that stands still', () => {
    const v5 = { ...createDefaultProfile(), version: 5 } as unknown as Record<string, unknown>;
